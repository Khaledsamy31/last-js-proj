...
const product2=new products({title: "car",price:200,sumPrice: 0,category:'accessories',img:'images/category-banner2.jpg',id:2});
const product3=new products({title: "bike",price:300,sumPrice: 0,category:'phones',img:'images/category-banner3.jpg',id:3});
const product4=new products({title: "home",price:400,sumPrice: 0,category:'cars',img:'images/category-banner4.jpg',id:4});
...
function addFavourite(id,e){
    ...
    mainId=id/100000;
if(localStorage.getItem('email')  && localStorage.getItem('firstName')){// to check is there a user
    if(ele.style.color=='red'){
        ...
        }),1) //to delete date from favo if fave color was red and clicked to remove  from favo
        localStorage.setItem('favourite',JSON.stringify(favourite));
    }else{//if user registered and clicked on favo
        ele.style.color='red' //change color to red
        favourite.push(product[id/100000])  //add in favourite array [] this id
        localStorage.setItem('favourite',JSON.stringify(favourite));
    }
}else{ // if there is no user and click on favo go to login page
    setTimeout(()=>{
        location.assign('login.html')
    },500)
}
